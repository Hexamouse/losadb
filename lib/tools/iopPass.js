 ...
 */
function EncryptDecryptData(szResultData, iResultSize, szSourceData, iSourceSize, bPassword) {
  const MAX_KEY = 30;
  const byKey = [
    [
 ...
 */
function EncryptPassword(szPassword) {
  result = [];
 ...
 */
function DecryptPassword(szResult) {
  const resultArr = szResult.split(",").map((x) => parseInt(x, 10));
  const szEncPassWord = Buffer.from(resultArr);
